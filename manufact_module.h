...
цикла. Период с самым большим номером соответствует периоду выхода готового продукта. **/
    ...
    private:
        ...
        size_t rcount;          // Количество позиций в номенклатуре сырья и материалов, требуемых для производства продукта
        strNameMeas *rnames;    // Одномерный массив с названиями сырья и материалов и единицами измерения. Размерность rcount
        decimal *recipeitem;    // Рецкптура продукта (одномерный аналог двумерной матрицы размером rcount*duration ).
                                ...
                                // сырья на производство единицы продукта в j-м периоде от начала его производства
    ...
    public:
        ...
        const size_t GetRCount() const;         // Возврат размера массива rnames (количество позиций сырья)
...
Балансовую полную и удельную стоимость незаконченного производства. **/
    ...
    private:
        ...
        decimal *RawMatPurchPlan;   /** Указатель на массив с планом закупок сырья и материалов в натуральном выражении по
        каждому виду сырья и материалов. Размерность массива rcount*PrCount **/
        decimal *RawMatPrice;       /** Указатель на массив с ценами на сырье и материалы в каждом периоде проекта. Размер
        массива rcount*PrCount **/
        strItem *Balance;           /** Указатель на массив со значениями объемов, балансовой стоимости незаконченного
...
реализуемость такой зависимости. **/
    ...
    private:
        size_t PrCount;                 // Количество периодов проекта
        size_t RMCount;                 // Полное количество позиций в номенклатуре сырья и материалов или используемых продуктов
        Currency hmcur;                 // Домашняя (основная) валюта ("Home Currency")
        strNameMeas *RMNames;           /** Полный массив с названиями сырья и материалов. Размерность RMCount. Этот массив в
