...
int main() {
    ...
    /** Перенаправляем вывод в текстовый файл   **/
        const string filename = "Report.txt";   // Имя файла для вывода отчетаs
    ...
    *asf = "123456789.0987654321000555";
    ...
    asf->View(cout);
    if(!asf->SaveToFile("asf.dat")) cout << "Ошибка записи в файл";
    delete asf;

    LongReal* arf = new LongReal;
    if(!arf->ReadFromFile("asf.dat")) cout << "Ошибка чтения из файла";
    cout << "Прочитанное число\n";
    arf->View(cout);
    delete arf;

    /** Ввод числа с терминала **/

    LongReal inNum;
    clog << "Введите число: ";
    cin >> inNum;
    clog << "\nВведенное число: " << inNum << endl;

    /** Прсиваивание переменным встроенных вещественных типов значений типа LongReal **/
