...
class clsRecipeItem {
/** Класс - рецептура отдельного продукта. Рецептура каждого продукта представляет собой матрицу: строки матрицы - это ресурсы,
...
удельный расход ресурсов на производство единицы данного продукта в натуральном выражении в указанный период производственного
цикла. Период с самым большим номером соответствует периоду выхода готового продукта. **/
    ...
    private:
        ...
        decimal *CalcWorkingValueItem(const size_t _PrCount, const size_t period, const decimal rmprice[], const decimal volume) const;
            /** Метод возвращает одномерный массив ресурсной себестоимости продукта в течении единичного технологического цикла.
            Продукт выпускается в заданный период period. Размерность возвращаемого массива _PrCount. Параметры: _PrCount
            - длительность проекта, period - период выпуска продукта, rmprice[] - 2D-массив с ценами ресурсов размерностью
        ...
        decimal *CalcWorkingBalanceItem(const size_t _PrCount, const size_t period, const decimal rmprice[], const decimal volume) const;
            ...
            Параметры: _PrCount - длительность проекта, period - период выпуска продукта, rmprice[] - 2D-массив с ценами ресурсов
        ...
        decimal *CalcProductBalanceItem(const size_t _PrCount, const size_t period, const decimal rmprice[], const decimal volume) const;
            ...
            - 2D-массив с ценами ресурсов размерностью rcount*PrCount, volume - объем выпускаемого продукта в период period. ВНИМАНИЕ!!!
        ...
        const string& _hmcur, strItem* (clsRecipeItem::*f)(const size_t, const decimal*, const strItem*) const) const;
            ...
            PrCount, rmprice[]  - 2D-массив с ценами ресурсов размерностью rcount*PrCount, где rcount - количество позиций
            ресурсов; strItem* (clsRecipeItem::*f)(const size_t, const decimal*, const strItem*) const - указатель на одну
            из функций CalcWorkingBalance или CalcProductBalance. **/
    ...
    public:
        ...
        strItem *CalcWorkingBalance(const size_t _PrCount, const decimal rmprice[], const strItem ProPlan[]) const;
            ...
            с ценами на ресурсы, размерностью rcount*PrCount. ВНИМАНИЕ!!! При вызове данного метода следует помнить о
        ...
        strItem *CalcProductBalance(const size_t _PrCount, const decimal rmprice[], const strItem ProPlan[]) const;
            ...
            продукта), rmprice матрица с ценами на ресурсы, размерностью rcount*PrCount. ВНИМАНИЕ!!! При вызове данного
        ...
        const strNameMeas* GetRefRawNamesItem() const;
            /** Метод возвращает константный указатель на внутренний массив с наименованиями ресурсов и единицами
            натурального измерения ресурсов. **/
...
/** -----------------------   Методы визуального контроля функций класса clsRecipeItem   ---------------------------------  **/
        ...
        const string& hmcur) const;
            ...
            2D-массив с ценами ресурсов размерностью rcount*PrCount, где rcount - количество позиций ресурсов,
...
class clsManufactItem {
/** Класс - производство отдельного продукта. Хранит данные о потребности в ресурсах для выпуска продукта в заданных
объемах по заданному графику, цены на ресурсы, объемы, полную и удельную себестоимость изготовленного продукта,
Балансовую полную и удельную стоимость незаконченного производства. **/
    ...
    private:
        ...
        size_t duration;            // Длительность производственного цикла
        size_t rcount;              // Количество позиций в номенклатуре ресурсов
    ...
    public:
        ...
        const size_t& GetRCount() const;     // Возвращает const-ссылку на количество позиций ресурсов, участвующего в производстве
        ...
        bool SetRawMatPrice(const decimal _Price[]);
            ...
            (матрицу) цен на ресурсы размером rcount*PrCount. **/

        bool MoveRawMatPrice(decimal* &_Price);
            /** Метод ввода цен на ресурсы. Предполагается, что после получения складом информации о потребности
            в ресурсах, склад возвращает информацию об учетных ценах на ресурсы. Эта информация с помощью
...
class clsManufactory {
/** Класс - производство всех необходимых для проекта продуктов. Позволяет осуществить расчет потребности в ресурсах
...
реализуемость такой зависимости. **/
    ...
    public:
        ...
        clsManufactory(const size_t _PrCount, const size_t _RMCount, strNameMeas* &_RMNames, const size_t msize,\
            ...
            /** Конструктор с параметрами: _PrCount - количество периодов проекта, _RMCount - полное количество позиций
            ресурсов, _RMNames - полный массив с названиями ресурсов, msize - полное количество наименований продуктов,
            _cur - валюта проекта, _Recipies - ссылка на указатель на массив с рецептурами продуктов. Наименования ресурсов и
        ...
        bool SetManufItem(const string &_name, const string &_measure, const size_t _duration, const size_t _rcount,\
            ...
            добавляет его в вектор, вводит список ресурсов и рецептуры в созданное производство. Параметры:
            _name - наименование продукта, _measure - наименование единицы измерения, _duration - длительность
            производственного цикла, _rcount - число позиций ресурсов, участвующих в производстве данного продукта,
            _rnames - массив с наименованиями ресурсов, размерностью rcount, являющихся подмножеством наименований из полного
            массива с названиями ресурсов, _recipe - указатель на 2D-массив с рецептурами, размером rcount*duration,
        ...
        bool SetRawMatPrice(const decimal _Price[]);
            ...
            потребности в ресурсах, склад возвращает информацию об учетных ценах на ресурсы.
        ...
        void CalcRawMatPurchPlan_future();
            /** Метод рассчитывает объем потребления ресурсов в натуральном выражении для всего плана
            выпуска всех продуктов. Все расчеты запускаются асинхронно в потоках, число которых на единицу
        ...
        const size_t GetRMCount() const;            // Метол возвращает общее число позиций ресурсов
        ...
        const strNameMeas *GetRMNames() const;
            /** Метод возвращает const-указатель на внутренний массив с именами и ед. измерения ресурсов. **/

        strNameMeas *GetRawMatDescription() const;
            /** Метод возвращает указатель на вновь создаваемый массив с именами и ед. измерения ресурсов. **/
        ...
        decimal *GetRawMatPurchPlan() const;
            ...
            содержится потребность для каждого наименования ресурса в каждом периоде проекта. Потребность в ресурсах суммарная,
        ...
        strItem *GetTotalProduct() const;
            /** Метод возвращает указатель на одномерный массив, являющийся аналогом двумерной матрицы с планами выхода всех
        ...
        const strNameMeas* GetRawNamesItem(const string& Name) const;
            /** Метод возвращает константный указатель на внутренний массив с наименованиями ресурсов и единиц их
            измерения в рецептуре продукта с именем Name **/

        const strNameMeas* GetRawNamesItem(const size_t _ind) const;
            /** Метод возвращает константный указатель на внутренний массив с наименованиями ресурсов в рецептуре продукта с
            индексом _ind **/
        ...
        const size_t GetRCount(const string& Name) const;
            /** Метод возвращает число позиций ресурсов, участвующих в рецептуре продукта с именем
            Name (размер массива с наименованиями ресурсов) **/

        const size_t GetRCount(const size_t _ind) const;
            /** Метод возвращает число позиций ресурсов, участвующих в рецептуре продукта с индексом _ind **/

        const strNameMeas GetNameItem(const size_t _ind) const;
            /** Метод возвращает наименование продукта и единицу его измерения для продукта с индексом _ind **/
