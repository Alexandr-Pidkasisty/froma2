...
/** Класс для хранения и совершения арифметических операций с длинными вещественными числами. **/
    ...
    public:
        ...
        explicit operator long double() const;           // Оператор присваивания переменной типа long double
        explicit operator double() const;                // Оператор присваивания переменной типа double
        explicit operator float() const;                 // Оператор присваивания переменной типа float
...
требуется промежуточная конвертация чисел LongReal в double и используется единый манипулятор для обоих типов. **/
    ...
    lrstream& operator<<(const string& val);    // Оператор вывода в поток lrstream строк std::string

    template<typename T, class=std::enable_if_t<are_types_for_lrstream_v<T>>>
    lrstream& operator<<(T val);     // Оператор вывода в поток lrstream используемых величин встроенных типов
