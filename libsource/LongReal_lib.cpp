...
/** Метод устанавливает значение числа равное положительной бесконечности. **/
    ...
    if(digits) delete[] digits;             // Если массив существует, то удаляем его
    digits = nullptr;                       // Устанавливаем указатель на массив в nullptr
    exponent = MaxEtype;                    // Присваиваем экспоненте максимально возможное число
}   // LongReal::setposinf
...
/** Метод устанавливает NaN. Если NaN установить не удалось, возвращает inf. **/
    sign = false;                           // Положительное число
...
/** Создает копию произвольного массива _digits и обрезает ее с конца, оставляя его размер равный Num.
Возвращает обрезанную копию **/
...
/** Обрезание внутреннего массива digits до нового размера, _n - новый размер **/
...
создает копию текущего экземпляра, увеличивает экспоненту числа в копии до значения _E не изменяя самого
...
Объявлены explicit для предотвращения неявных (автоматических) преобразований типов **/
...
stringstream& operator>>(stringstream& ss, LongReal& value) {
...
lrstream& lrstream::operator<<(const string& val) {
/** Оператор вывода в поток lrstream строк std::string **/
    *pos << val;
    return *this;
}
