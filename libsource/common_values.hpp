...
const decimal   epsln = 1e-9;   // Бесконечно малая величина, являющаяся альтернативой нулю
...
enum Currency{Currency_MIN, RUR, USD, EUR, CNY, Currency_MAX};  // Тип валюты (российский рубль, доллар США, Евро,
                                                                // китайский юань. Может дополняться другими валютами
const string CurrencyTXT[] = {"", "RUR", "USD", "EUR", "CNY", ""};  // Представление валют в виде текста.
                                                                // При расширении списка типа Currency, должен
                                                                // дополняться другими валютами
...
функции copy_n. Проверка осуществляется на этапе компиляции (constexpr). Тривиально копируемый тип
(is_trivially_copyable) - это тип, хранилище которого является непрерывным и, следовательно, его копирование
подразумевает тривиальную копию блока памяти, как если бы это выполнялось с помощью memcpy. Это справедливо
для скалярных типов, тривиально копируемых классов и массивов любых подобных типов.
